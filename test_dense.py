...
import time
from model.GRNet_dense import GPCC_upsampling
...
def parse_args():
  ...
  parser.add_argument("--ckpts", default='./ckpts_pretrain/dense/r01/last_epoch.pth', help='Path to pretrained model')
  ...
  parser.add_argument("--last_kernel_size", type=int, default=7,help='The final layer kernel size, R01-R05:k=7')
  parser.add_argument("--resolution", type=int,default=4095, help='Follow MPEG CTC ,9bit:511, 10bit:1023, 11bit:2047, 12bit:4095')
  parser.add_argument("--posQuantscale", type=float, default=0.03125, help='PosQuantscale of G-PCC(octree), R01:0.03125, R02:0.0625,'
...
model = GPCC_upsampling(last_kernel_size=args.last_kernel_size).to(device)
...
for  pc_gt in GT_files:
    ...
    for j,part_gt in enumerate(parts_gt):
        ...
        ####################### GRNet ####################
        with torch.no_grad():
            out, out_cls, target, keep, out_offset = model(x, coords_T=p2, device=device,train_offset=True, prune=True)
        ##################################################
        run_time += time.time() - start_time
        gpcc_list.append(x.C[:, 1:])
        out_list.append(out_offset.squeeze(0))
